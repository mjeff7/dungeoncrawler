...
function cellType(x, y) {
  if(wp.player.location.isAt(x, y))
    return 'PLAYER';
  if(cellDistanceFromPlayer(x, y) >= 2 + wp.player.getLevel())
    return 'BLACK';
  const lists =
        [{list: wp.items, result: 'ITEMS'},
         {list: wp.baddies, result: 'BADDIES'}];
...
function pickupItem(item) { return item.pickupItem(); }
function collideWith(item, removeFromMap) { return item.collideWith(removeFromMap); }

const movePlayerBy = (dx, dy) => movePlayerTo(wp.player.location.offsetBy(dx, dy));
function movePlayerTo(x, y) {
  if(x instanceof Location)
    return movePlayerTo(x.x, x.y);
  ...
  if(!playerBlocked)
    wp.player.location.set(x, y);
  
  if(wp.locatables.filter(x => x.item instanceof Baddie).toArray().length === 0)
    alert("You win!");
...
function initWorld() {
  let items = util.rangeCollect(0, 30, i =>
                                new Weapon('dagger ' + i,
                                           {min: Math.floor(Math.pow(2, i / 3)),
                                            max: Math.floor(Math.pow(2, (i+2) / 3))}));
  items = [new Weapon('Dagger', {min: 2, max: 10}),
           new Weapon('Sword', {min: 5, max: 20}),
           new Weapon('Nunchucks', {min: 0, max: 15})];
...
class WorldView extends React.Component {
  ...
  render() {
    return <div className="gameView">
      <DungeonView world={this.props.world}/>
      <GameStatus world={this.props.world}/>
      <div className="status">
        <div>
          Player: {JSON.stringify(this.props.world.player)}, level: {this.props.world.player.getLevel()}
        </div>
        <div>
          Baddies: {this.props.world.locatables.filter(i => i.item instanceof Baddie).toArray().map(b => b[0].item.attacking.health + ", ").toString()}
        </div>
      </div>
    </div>;
  }
...
function PlayerStat(props) {
  return <div className="playerStat">
...
function GameStatus(props) {
  ...
  return <div className="gameStatus">
    <PlayerStat title="Player health" value={w.player.attacking.health}/>
    <PlayerStat title="Weapon" value={`${weapon.name} ${weapon.attackStrength.min}–${weapon.attackStrength.max}`}/>
    <PlayerStat title="XP" value={w.player.xp}/>
    <PlayerStat title="Level" value={w.player.getLevel()}/>
    </div>;
}

const HEIGHT=300, WIDTH=HEIGHT,
...
function DungeonViewSVG(props) {
  return <svg width={WIDTH} height={HEIGHT}>
    <circle/>
    {props.world.map.tiles.map((line, ycell) => <g>{
      line.map((cell, xcell) => <rect
                                  x={x(xcell)}
                                  y={y(ycell)}
...
function DungeonView(props) {
  return <div className="squareWrapper"><div className="squareSpacer"/><div className="dungeonView">
    {props.world.map.tiles.map((line, index) => <DungeonLine key={index} line={line} index={index}/>)}
  </div></div>;
}

function DungeonLine(props) {
  return <div className="dungeonLine">{props.line.map((_,i) => <div key={i} className={'TILE ' + cellType(i, props.index)} style={{opacity: 1-0*cellDistanceFromPlayer(i, props.index) / (10 + wp.player.getLevel())}}/>)}</div>;
}

let reactUpdate;
ReactDOM.render(<WorldView world={wp} updateFunc={f => reactUpdate = f}/>, document.getElementById('reactRoot'));
